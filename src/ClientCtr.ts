...
const ClientCtr = {
    ...
    getSuppliersByNameContaining: (key: string) => axios.get(baseURL + '/supplier?key=' + key),
    getSupplier: (id: number) => axios.get(baseURL + '/supplier/' + id),
    saveSupplier: (s: any) => axios.post(baseURL + '/supplier', s),
    deleteSuppliersById: (arr: number[]) => axios.delete(baseURL + '/supplier', {data: arr}),
